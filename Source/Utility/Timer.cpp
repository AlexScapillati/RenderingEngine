...
float Timer::GetTime() const
{
	...
	if (mHighRes)
	{
		...
		const auto dTime = static_cast<double>(newHighResTime.QuadPart - mHighResStart) / static_cast<double>(mHighResFreq);
...
float Timer::GetLapTime()
{
	...
	if (mHighRes)
	{
		...
		const auto dTime = static_cast<double>(newHighResTime.QuadPart - mHighResLap) / static_cast<double>(mHighResFreq);
		fTime = static_cast<float>(dTime);
