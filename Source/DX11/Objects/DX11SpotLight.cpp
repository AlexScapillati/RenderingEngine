...
namespace DX11
{
	CDX11SpotLight::CDX11SpotLight(CDX11Engine* engine, std::string mesh, std::string  name, std::string& diffuse,
		CVector3    colour, float       strength, CVector3 position, CVector3 rotation,
		float       scale)
		: CDX11Light(engine,
			mesh,
			name, diffuse, colour, strength, position,
			rotation, scale)
	{
	...
	CDX11SpotLight::CDX11SpotLight(CDX11SpotLight& s) : CDX11Light(s)
	{
	...
	void CDX11SpotLight::Render(bool basicGeometry)
	{
	...
	ID3D11ShaderResourceView* CDX11SpotLight::RenderFromThis()
	{
		...
		gPerFrameConstants.viewMatrix = InverseAffine(CGameObject::WorldMatrix());
	...
	void CDX11SpotLight::SetShadowMapsSize(int value)
	{
		//TODO boundaries
	...
	void CDX11SpotLight::SetConeAngle(float value)
	{
		//TODO boundaries
		mConeAngle = value;
	}
	...
	CDX11SpotLight::~CDX11SpotLight()
	{
		Release();
	}

	void CDX11SpotLight::Release() const
	{
