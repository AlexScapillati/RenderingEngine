	...
	void CDX11Scene::DisplayPostProcessingEffects()
	{
		...
		if (choosePP)
		{
			if (ImGui::Begin("Choose"))
			{
				...
				for (const auto& s : mPostProcessStrings)
				{
					if (s == "Copy") continue;
					items += s + '\0';
				}
				...
				for (const auto& s : mPostProcessModeStrings)
				{
					modes += s + '\0';
				}
		...
		if (editProperties && mPostProcessingFilters.size())
		{
			...
			{
				//for each effect
				for (const auto& postProcess : mPostProcessingFilters)
				{
