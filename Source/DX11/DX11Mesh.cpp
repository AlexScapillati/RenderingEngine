...
namespace DX11
{
	...
	CDX11Mesh::CDX11Mesh(CDX11Engine* engine, const std::string& fileNameN, bool requireTangents /*= false*/)
	{
	...
	CDX11Mesh::CDX11Mesh(const CDX11Mesh& mesh) : CDX11Mesh(mesh.mEngine, mesh.mFileName, mesh.hasTangents) {}

	//--------------------------------------------------------------------------------------

	// Helper function for Render function - renders a given sub-mesh. World matrices / textures / states etc. must already be set
	void CDX11Mesh::RenderSubMesh(const SubMesh& subMesh)
	{
	...
	void CDX11Mesh::Render(std::vector<CMatrix4x4>& modelMatrices)
	{
	...
	unsigned int CDX11Mesh::CountNodes(aiNode* assimpNode)
	{
	...
	unsigned int CDX11Mesh::ReadNodes(aiNode* assimpNode, unsigned int nodeIndex, unsigned int parentIndex)
	{
