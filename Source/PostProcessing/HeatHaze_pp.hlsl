...
float4 main(PostProcessingInput input) : SV_Target
{
	...
	float alpha = 1.0f - saturate((centreLengthSq - 0.25f + heatSoftEdge) / heatSoftEdge); // Soft circle calculation based on fact that this circle has a radius of 0.5 (as area UVs go from 0->1)
	...
	float2 hazeOffset = float2(SinY, SinX) * heatEffectStrength * alpha * gArea2DSize;
